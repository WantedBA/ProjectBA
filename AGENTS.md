...
- 작업 도중에는 검토와 되돌리기 편의를 위해 세부 커밋을 남길 수 있지만, 원격 push 전에는 사용자의 지시에 따라 관련 커밋을 큰 기능 단위로 squash/rebase해 이력을 정리한다.
- push 전 커밋 정리를 할 때는 합칠 커밋 범위와 남길 커밋 메시지를 먼저 확인하고, 기능 변경/데이터 변경/에디터 에셋 변경처럼 성격이 다른 작업은 필요한 경우 분리해 둔다.
